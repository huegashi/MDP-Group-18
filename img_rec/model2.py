...
def load_model():
    ...
    model = YOLO("C:/Users/gauth/OneDrive - Nanyang Technological University/Documents/SC2079//MDP-FINAL-main_2/MDP-Algorithm/Task2.pt")
    return model
...
def predict_image(image, model, signal):
    ...
    try:
        ...
        preds = [p for p in preds if p["name"] not in ["bullseye", "arrow-bullseye", "bullseye-arrow-bullseye", "99", "Bullseye"]]
        if not preds:
            return "NA"
        ...
        try:
            ...
            # Find the latest detect folder inside runs (e.g., detect, detect2, detect3, etc.)
            detect_folders = [
                os.path.join(runs_dir, d) for d in os.listdir(runs_dir)
                if d.startswith("detect") and os.path.isdir(os.path.join(runs_dir, d))
            ]

            if not detect_folders:
                raise FileNotFoundError("No detection folders found in 'runs/'")

            # Get the most recently modified detect folder
            latest_detect = max(detect_folders, key=os.path.getmtime)

            # Construct full path to the detected image
            detected_image_path = os.path.join(latest_detect, "image0.jpg")

            # Open in VSCode
            os.system(f'code "{detected_image_path}"')

        except Exception as e:
            print(f"Could not open file in VSCode: {e}")

        # Return YOLO class id as string (no manual mapping)
        return str((pred["class"]))
...
# --- at top of file if you like (optional convenience) ---
...
def stitch_image():
    ...
    # Safer glob on Windows
    imgPaths = glob.glob(os.path.join("runs", "detect   *", "*.jpg"))

    if not imgPaths:
        # Nothing to stitch; return a tiny blank image or raise a clear error
        # raise FileNotFoundError("No .jpg files found in runs/detect/*/")
    ...
    # Move originals AFTER ensuring destination exists
    for img in imgPaths:
